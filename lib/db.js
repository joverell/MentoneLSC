...
export function getDb() {
  ...
  if (!singleton.db) {
    const sourceDbPath = path.join(process.cwd(), 'mentone.db');
    const writableDbPath = '/tmp/mentone.db';

    // Copy the database to the writable /tmp directory if it doesn't exist there yet.
    // This is done once per serverless function instance.
    if (!fs.existsSync(writableDbPath)) {
      ...
      console.log(`Database copied to ${writableDbPath}`);
    }

    console.log("Opening database connection from writable location...");
    singleton.db = new Database(writableDbPath);
